...
func GetShiftSummaryHandler(ctx *gin.Context) {
	var request entities_reports.ShiftSummaryRequest
	if err := ctx.ShouldBindJSON(&request); err != nil {
		ctx.JSON(http.StatusBadRequest, entities_main.NewErrorResponse[interface{}]("Body inválido", err))
		return
	}

	if request.Pos == 0 || request.FechaInicio == "" || request.FechaFin == "" {
		ctx.JSON(http.StatusBadRequest, entities_main.NewErrorResponse[interface{}]("pos, fecha_inicio y fecha_fin son requeridos", nil))
		return
	}
	...
	response, err := container_reports.ResolveGetShiftSummaryContainer().Execute(
		request.Pos,
		request.FechaInicio,
		request.FechaFin,
	)
