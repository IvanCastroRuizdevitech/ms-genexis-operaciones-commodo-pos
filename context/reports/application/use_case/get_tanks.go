...
var _ iusecase.IGetTanks = (*GetTanks)(nil)

type GetTanks struct {
	Repository irepositories.IGetTanksRepository
}

func (u *GetTanks) Execute() (*entities_main.Response[[]entities_reports.TankBodega], error) {
	result, err := u.Repository.GetAll()
	if err != nil {
		log.Println("GetTanks error:", err)
		return nil, err
	}
