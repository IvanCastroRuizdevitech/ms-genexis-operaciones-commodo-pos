...
func GetDispenserDetailsByFamiliesHandler(ctx *gin.Context) {
	var request entities_creditcustomers.DispenserDetailsByFamiliesRequest
	if err := ctx.ShouldBindJSON(&request); err != nil {
		ctx.JSON(http.StatusBadRequest, entities_main.NewErrorResponse[interface{}]("Body invalido", err))
		return
	}

	if len(request.FamiliesIDs) == 0 {
		ctx.JSON(http.StatusBadRequest, entities_main.NewErrorResponse[interface{}]("families_ids es requerido", nil))
		return
	}

	response, err := container_creditcustomers.ResolveGetDispenserDetailsByFamiliesContainer().Execute(&request)
	if err != nil {
