...
func UpdateTransactionByAuthorizationHandler(ctx *gin.Context) {
	var request entities_creditcustomers.UpdateTransactionByAuthorizationRequest
	if err := ctx.ShouldBindJSON(&request); err != nil {
		ctx.JSON(http.StatusBadRequest, entities_main.NewErrorResponse[interface{}]("Body invalido", err))
		return
	}

	response, err := container_creditcustomers.ResolveUpdateTransactionByAuthorizationContainer().Execute(&request)
	if err != nil {
