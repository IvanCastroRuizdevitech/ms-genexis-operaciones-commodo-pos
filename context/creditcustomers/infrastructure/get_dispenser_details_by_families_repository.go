...
type GetDispenserDetailsByFamiliesRepository struct {
	Connection infrastructure_db_client.DatabaseConnectionInterface
}

func (r *GetDispenserDetailsByFamiliesRepository) Get(request *entities_creditcustomers.DispenserDetailsByFamiliesRequest) (*entities_main.Response[entities_creditcustomers.DispenserDetailsFunctionResponse], error) {
	...
	log.Println("CONSULTANDO:", constants_creditcustomers.QUERY_GET_DISPENSER_DETAILS_BY_FAMILIES)

	err = conn.PgxConn.QueryRow(
		context.Background(),
		constants_creditcustomers.QUERY_GET_DISPENSER_DETAILS_BY_FAMILIES,
		request.FamiliesIDs,
	).Scan(&jsonResult)
	if err != nil {
		return nil, err
	...
	if err := json.Unmarshal([]byte(jsonResult), &payload); err != nil {
		log.Println("[GetDispenserDetailsByFamiliesRepository][Unmarshal]", err)
		return nil, err
	}
