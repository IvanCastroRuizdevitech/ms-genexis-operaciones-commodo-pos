...
// ActiveDispenserFaceTransactionRequest maps to genexis_operaciones.fnc_transaccion_surtidor_cara_vigente
// It either fetches an existing active transaction or creates one if missing.
type ActiveDispenserFaceTransactionRequest struct {
	...
	ProveedorID    int            `json:"proveedores_id" binding:"required"`
	MontoMaximo    float64        `json:"monto_maximo" binding:"required"`
	CantidadMaxima float64        `json:"cantidad_maxima" binding:"required"`
	Trama          map[string]any `json:"trama" binding:"required"`
