...
type CheckReadySalesRepository struct {
    Connection infrastructure_db_client.DatabaseConnectionInterface
}

func (r *CheckReadySalesRepository) Check(request *entities_sales.CheckReadySalesRequest) (*entities_main.Response[[]entities_sales.PendingSale], error) {
    log.Println("CONSULTANDO:", constants_sales.QUERY_CHECK_READY_SALES)
    ...
    if err := r.Connection.Select(
        context.Background(),
        &rows,
        constants_sales.QUERY_CHECK_READY_SALES,
        request.JournalId,
        request.PromoterId,
        request.Limit,
    ); err != nil {
